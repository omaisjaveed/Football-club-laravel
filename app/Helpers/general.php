...
if (!function_exists('productsByBrand')) {
	function productsByBrand(array $brandNames)
	{
		$brands = \App\Entity\Brand\Brand::whereIn('slug', $brandNames)
			->with(['products.image', 'products.translation'])
			->get();

		// Map each brand to its products for easy access
		$productsByBrand = [];
		foreach ($brands as $brand) {
			$productsByBrand[$brand->slug] = $brand->products;
		}
