									...
									<tbody>
										@foreach($addresses as $customeraddress)
											<tr data-id="row_{{ $customeraddress->id }}">
												...
												<td class='address'>
													<strong><u>{{ ucwords($customeraddress->type) . ' ' . _lang('Address') }}
															{{ $customeraddress->is_default == 1 ? '(' . _lang('Default') . ')' : '' }}</u></strong><br>
												...
												<td class="text-center">
													<a href="{{ url('/update_address/' . $customeraddress->id) }}"
														class="text-info"><i class="fa fa-pencil-square-o"
															aria-hidden="true"></i></a>
													<a href="{{ url('/delete_address/' . $customeraddress->id) }}"
														class="text-danger"><i class="fa fa-trash-o class=" btn
															btn-primary"" aria-hidden="true"></i></a>
												</td>
											</tr>
										@endforeach
									</tbody>
