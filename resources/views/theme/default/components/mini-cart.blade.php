...
<a href="#" class="single-icon"><i class="ti-shopping-cart"></i>
...
<div class="shopping-item">
	...
	<ul class="shopping-list">
		...
		@foreach($cart_items as $cart)
			<li>
				<a href="{{ url('/remove_cart_item/' . $cart->id) }}" class="remove"
				...
				<h4>
					<a
						href="{{ url('product/' . $cart->associatedModel->slug) }}">{{ $cart->associatedModel->translation->name }}</a>
				</h4>
				...
				<p class="quantity">{{ $cart->quantity }}x - <span
						class="amount">{!! xss_clean(show_price($cart->price)) !!}</span></p>
			</li>
		@endforeach
	</ul>
	<div class="bottom">
